...
class ChangeIPForm(forms.SelfHandlingForm):
    ...
    fixed_ip = forms.IPField(
        ...
        required=False,
        help_text=_("IP address for the new port. Leave empty for auto-assignment."),
    ...
    actions = forms.ThemableChoiceField(
        ...
        required=False,
        help_text=_("Select an action to perform based on Current and New IP Addresses"),
        choices=[
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        for port in self.ports:
            network_name = list(filter(lambda network: network.id == port.network_id, self.networks))[0].name
            for ip in port.fixed_ips:
                choice_id = _('%s__split__%s') % (port.id, ip['ip_address'])
                choices.append((choice_id, _('%s (%s)') % (ip['ip_address'], network_name)))
    ...
    def apply_networking(self, request, instance_id, selected_port_id, selected_port_ips, success_message):
        try:
            ...
            self.reverse_networking(request, selected_port_id, selected_port_ips)
            return False

    def reverse_networking(self, request, selected_port_id, fixed_ips):
        try:
            api.neutron.port_update(request, selected_port_id, fixed_ips=fixed_ips)
        except Exception as e:
    ...
    def handle(self, request, data):
        ...
        selected_port_ips = list(filter(lambda port: port.id == selected_port_id, self.ports))[0].fixed_ips or []
        if selected_port_ips:
            selected_port_ips = list(map(lambda ip: {'ip_address': ip['ip_address']}, selected_port_ips))

        if action == 'change':
            new_selected_port_ips = list(filter(lambda ip: ip['ip_address'] != selected_port_ip, selected_port_ips))
            new_selected_port_ips.append({'ip_address': fixed_ip})
            try:
                api.neutron.port_update(request, selected_port_id, fixed_ips=new_selected_port_ips)
            except Exception as e:
                messages.error(request, _("Unable to change port. %s") % e)
                return False

            return self.apply_networking(request, instance_id, selected_port_id, selected_port_ips,
        ...
        if action == 'remove':
            ...
            try:
                ...
                messages.error(request, _("Unable to remove IP address. %s") % e)
                return False
